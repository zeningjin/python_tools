            ...
            for project_demand_id, project_demand_data in project_demand_datas.items():
                ...
                if project_demand_data['project_demand_stage'] == '已发布':
                    ...
                    for i, pro_remark in pro_remarks.items():
                        if pro_remark.get('update_context_end') == 'released' and pro_remark.get('update_time') > new_pro_time:
                ...
                if project_demand_data['project_demand_stage'] == '研发中':
                    ...
                    for i, pro_remark in pro_remarks.items():
                        # 有过测试状态
                        if pro_remark.get('update_context_end') == 'tested' and pro_remark.get('update_time') <= new_pro_time:
                            test_people = pro_remark.get('update_people')
                        if pro_remark.get('update_context_end') == 'developing' and pro_remark.get('update_time') > new_pro_time:
                            update_people = pro_remark.get('update_people')
                    if test_people:
                        pro_time = (CommonTools.get_work_days(datetime.datetime.now() - datetime.timedelta(days=1))).strftime("%Y-%m-%d")
                        pro_insert_data.append([
...
if __name__ == '__main__':
    # 发送上线结果文档
    pre_insert_data = test_object.get_pre_data_by_project_demand_datas(all_project_demand_datas)
    SendEmail.send_pre_email(pre_insert_data, receiver_list)
